...
class RandoopRun:
    def __init__(self, unit_tests_name, unit_tests_directory, CUT_name, classlist_filename, timelimit, paths, randoop_only, dont_terminate = False, use_concrete_values = False, seed = 0, dependencies_classpath = None):
    ...
    def run(self):
        ...
        additional_params += " --forbid-null=false --small-tests=true --testsperfile=1000 --ignore-flaky-tests"
        ...
        if self.dont_terminate:
            ...
            command_str = "java $JVM_FLAGS -ea -cp " + cp + " randoop.main.Main gentests --testclass=" + self.CUT_name + " --junit-output-dir=" + self.unit_tests_directory + " --regression-test-basename=" + self.unit_tests_name + " --timelimit=%s" % self.unit_tests_timelimit + additional_params
...
class JDoop:
    ...
    def run_randoop(self, unit_tests, CUT_name, classlist, timelimit, dont_terminate = False, use_concrete_values = False, seed = 0):
        """Invokes Randoop"""

        if self.randoop_only:
            seed = 0
        randoop_run = RandoopRun(unit_tests.name, unit_tests.directory, CUT_name, classlist.filename, str(timelimit), self.paths, self.randoop_only, dont_terminate, use_concrete_values, seed, self.dependencies_classpath)
        randoop_run.run()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--testclass', help='Name of a CUT')
    parser.add_argument('--junit-output-dir', help='The dir of output files')
    ...
    # CUT_name
    CUT_name = params.testclass

    # unit_tests_directory = params.junit-output-dir

    # Invoke Randoop to generate unit tests
    init_seed = 10
    jdoop.start_clock("Randoop #1")
    jdoop.run_randoop(unit_tests, CUT_name, classlist, timelimit, dont_terminate = True, seed = init_seed)
    ...
    while not params.randoop_only:
        ...
        if timelimit > 3:
            ...
            if params.randoop_only:
                jdoop.run_randoop(unit_tests, CUT_name, classlist, timelimit, dont_terminate = True, seed = init_seed + i)
            else:
                jdoop.run_randoop(unit_tests, CUT_name, classlist, timelimit, dont_terminate = True, use_concrete_values = True, seed = init_seed + i)
