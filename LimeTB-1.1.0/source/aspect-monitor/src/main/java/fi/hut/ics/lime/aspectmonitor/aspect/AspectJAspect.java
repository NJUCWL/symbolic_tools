...
//				perClause.getPointcut().toString().replace("call(", "execution(") + ";\n");
		...
		// see http://www.mail-archive.com/aspectj-users@eclipse.org/msg04092.html
		// (and generally the thread)
		string += "    static class Thrower {\n";
		string += "        private static Throwable throwable;\n";
		string += "        private Thrower() throws Throwable {\n";
		string += "            throw throwable;\n";
		string += "        }\n";
		string += "        public static void $throw(Throwable t) {\n";
		string += "            throwable = t;\n";
		string += "            try {\n";
		string += "                Thrower.class.newInstance();\n";
		string += "            } catch (InstantiationException e) {\n";
		string += "                throw new Error(\"barf: InstantiationException\");\n";
		string += "            } catch (IllegalAccessException e) {\n";
		string += "                throw new Error(\"barf: IllegalAccessException\");\n";
		string += "            }\n";
		string += "        }\n";
		string += "    }\n";
		string += "\n";
