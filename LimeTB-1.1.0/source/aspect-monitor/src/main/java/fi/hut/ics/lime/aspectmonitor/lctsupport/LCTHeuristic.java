...
public class LCTHeuristic {
	...
	public static int decayingBest() {
		...
		Set<Object> observers = counter.getObservers();
		Map<Object, Double> expectedSteps = counter.getExpectedSteps();
		Map<Object, Integer> expectedStepsAges = counter.getExpectedStepsAges();
		...
		if (observers.size() == 0) {
			...
			for (Object observer : observers) {
				double decayedValue = expectedSteps.get(observer) + expectedStepsAges.get(observer);
