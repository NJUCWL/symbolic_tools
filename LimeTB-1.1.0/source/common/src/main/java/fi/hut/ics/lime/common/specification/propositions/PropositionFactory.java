...
public class PropositionFactory {
	...
	private static final Pattern CALL_PROPOSITION_PATTERN = Pattern.compile("^([a-z][a-zA-Z]*) ::= ([a-z][a-zA-Z]*)"+Pattern.quote("()"));
	private static final Pattern VALUE_PROPOSITION_PATTERN = Pattern.compile("^([a-z][a-zA-Z]*) ::= ([^;]+)");
	private static final Pattern EXCEPTION_PROPOSITION_PATTERN = Pattern.compile("^([a-z][a-zA-Z]*) ::= ([a-zA-Z_$][a-zA-Z_0-9.]*)");
	 ...
	 */
	public static PropositionFactory instance() {
		return instance;
	}
	...
	public Map<String, PropositionNode> getPropositionNodesByType(Source sourceType) {
		return propositionNodes.get(sourceType);
	}
	...
	throws PropositionException {
		return new Proposition(node, precount++);
	}
	...
	throws PropositionException {
		...
		Map<String, PropositionNode> propMap = propositionNodes.get(sourceType);
		
		if(propMap == null) {
			propMap = new Hashtable<String, PropositionNode>();
			propositionNodes.put(sourceType, propMap);
		}
