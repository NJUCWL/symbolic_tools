...
public class FileUtils {
	 ...
	 * @throws IOException
	 */
	private static void copyFile(File sourceFile, File target) 
													throws IOException {
		
		assert sourceFile.isFile() : "sourceFile does not exist or is " +
		 "not a file.";
		
		if (target.isDirectory()) {
			/* Specify the target to be a file in the target directory with
			 * the same name as the source file. */
			String targetDir = target.getCanonicalPath().trim();
			
			// Add the trailing / (Unix) or \ (Windows), if missing
			if (!targetDir.endsWith(File.separator)) {
				targetDir += File.separator;
			}
	
			target = new File(targetDir + sourceFile.getName());		
		}
		
		// target should now represent an existing or a non-existing file
		if (!target.createNewFile() && !(target.delete() && 
										target.createNewFile())) {
			// Could not create file or delete existing one
			throw new IOException("ERROR: Unable to delete or create " 
	...
	/* Non-recursively (doesn't copy subdirectories) copies all the 
	 * files from sourceDir to targetDir.
	 *  
	 * @param sourceDir the directory to copy files from
	 * @param targetDir the directory to copy files to
	 * @throws IOException
	 ...
	 */
	public static void writeFile(File toWrite, String contents) 
													throws IOException {
