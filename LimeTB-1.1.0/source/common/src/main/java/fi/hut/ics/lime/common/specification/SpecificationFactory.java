...
public class SpecificationFactory {
	...
	private static final PropositionFactory propFactory = PropositionFactory.instance();
	...
	private static final Pattern PROPERTY_PATTERN = Pattern.compile("^([a-zA-Z_]\\w*) ::= (.+)");	
	...
	throws SpecificationException {
		...
		if (scanner.match().groupCount() != 2)
			throw new AssertionError("PROPERTY_PATTERN should have exactly 2 groups.");
		specification.setName(scanner.match().group(1));
		specification.setDeclared(scanner.match().group(2));
	}
