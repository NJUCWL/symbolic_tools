...
static void reset_bfr (BtorFormatReader * bfr) {
  int i;
  assert (bfr);
...
void delete_btor_format_reader (BtorFormatReader * bfr) {
  reset_bfr (bfr);
  free (bfr);
}

static int getc_bfr (BtorFormatReader * bfr) {
...
static int perr_bfr (BtorFormatReader * bfr, const char * str) {
  assert (!bfr->error);
  bfr->error = malloc (strlen (str) + 20);
  sprintf (bfr->error, "line %d column %d: %s",
           bfr->errcoo.x, bfr->errcoo.y, str);
  return 0;
}

static void pushc_bfr (BtorFormatReader * bfr, int ch) {
...
do { \
  if (!strcmp (bfr->buf, STR)) { line.tag = TAG; goto TAGDONE; } \
} while (0)

static int readl_bfr (BtorFormatReader * bfr) {
  BtorFormatLine line;
  int ch;
START:
  bfr->errcoo = bfr->coo;
  ...
  if (ch == ';') {
    while ((ch = getc_bfr (bfr)) != '\n')
      if (ch == EOF)
	return perr_bfr (bfr, "unexpected end-of-file in comment");
    goto START;
  }
...
static void pushl_bfr (BtorFormatReader * bfr, BtorFormatLine * l) {
...
const char * error_btor_format_reader (BtorFormatReader * bfr) {
  return bfr->error;
}
