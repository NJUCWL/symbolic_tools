...
do { (ptr) = (typeof(ptr)) \
       btor_malloc ((mm), (nelems) * sizeof *(ptr)); } while (0)

#define BTOR_CNEWN(mm,ptr,nelems) \
do { (ptr) = (typeof (ptr)) \
       btor_calloc ((mm), (nelems), sizeof *(ptr)); } while (0)

#define BTOR_CLRN(ptr,nelems) \
do { memset ((ptr), 0,  (nelems) * sizeof *(ptr)); } while (0)

#define BTOR_DELETEN(mm,ptr,nelems) \
do { btor_free ((mm), (ptr), (nelems) * sizeof *(ptr)); } while (0)

#define BTOR_REALLOC(mm,p,o,n) \
do { \
  (p) = (typeof (p)) \
    btor_realloc ((mm),(p),((o)*sizeof*(p)),((n)*sizeof*(p))); \
} while (0)
...
void *btor_sat_realloc (BtorMemMgr *mm, void *, size_t oldsz, size_t neusz);

void btor_sat_free (BtorMemMgr *mm, void *p, size_t freed);

void *btor_malloc (BtorMemMgr * mm, size_t size);

void *btor_realloc (BtorMemMgr * mm, void *, size_t oldsz, size_t neusz);
