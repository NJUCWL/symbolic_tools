...
"  -d2fun, -ds2fun, --dump-smt2-fun dump expression in SMT 2.0 format using\n"
  ...
  if (!app.done && !app.err)
...
#ifndef NBTORLOG
      if (!app.loglevel && getenv ("BTORLOG"))
	app.loglevel = 1;
      ...
      else
	{
	  ...
	  if (ch != EOF && ch)
		  ...
		  if (second == 'b')
		    {
		      parser_api = btor_smt_parser_api ();
		      if (app.verbosity > 0)
			btor_msg_main_va_args ("assuming SMTLIB version 1 " \
			...
			{
			if (isprint (second))
			  btor_msg_main_va_args ("assuming SMTLIB version 2 " \
		...
		btor_msg_main_va_args ("assuming BTOR parsing because first " \
				       "character differs from '('\n");
	    }
	  else if (app.verbosity > 0)
	    {
	      if (ch == EOF)
		btor_msg_main_va_args ("assuming BTOR parsing because " \
				       "end-of-file found\n");
	      else
		{
		  assert (!ch);
      ...
      else if (app.dump_smt)
	{
	  ...
	  if (app.rewrite_level >= 2)
		...
		btor_dump_smt2_fun_after_global_rewriting (btor,
							   app.output_file);
	    }
	  else
	    {
	      ...
	      for (i = 0; i < parse_res.noutputs; i++)
		{
	      ...
	      if (app.dump_smt <= 1)
		btor_dump_smt1 (btor, app.output_file, &all,
				parse_res.noutputs);
	      else if (app.dump_smt == 2)
		btor_dump_smt2 (btor, app.output_file, &all,
				parse_res.noutputs);
	      else
		btor_dump_smt2_fun (btor, app.output_file, &all,
				    parse_res.noutputs);
