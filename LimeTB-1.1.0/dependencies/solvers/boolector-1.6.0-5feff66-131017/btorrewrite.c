		    ...
		    BtorNode * e0, BtorNode * e1)
{
  ...
  (kind == BTOR_BEQ_NODE || kind == BTOR_AEQ_NODE || kind == BTOR_ADD_NODE))
    {
      if (kind == BTOR_BEQ_NODE)
	{
	  if (e0 == e1)
...
btor_rewrite_apply_exp (Btor * btor, BtorNode * fun, BtorNode * args)
{
  ...
  inv_result = 0;
//  printf ("rewrite apply: %s, %s\n", node2string (cur_fun), node2string (cur_args));
  // TODO: new limit?
...
//	  printf ("    is_true: %d\n", is_true_cond (beta_cond));
	  ...
	  if (!BTOR_REAL_ADDR_NODE (cur_branch)->parameterized)
	      ...
	      if (btor_param_cur_assignment (cur_branch))
		{
		  result =
		    btor_copy_exp (btor,
				   btor_param_cur_assignment (cur_branch));
		}
	      else
		result = btor_copy_exp (btor, cur_branch);
	  ...
	  if (fptr)
		       ...
		       fptr != btor_rewrite_urem_exp)
		{
		  ...
		  if (e_if->e[0] == e_else->e[1])
			...
			btor_rewrite_add_exp, e_if->kind);
		      BTOR_INC_REC_RW_CALL (btor);
